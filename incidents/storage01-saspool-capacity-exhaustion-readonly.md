...
1. **~2.2T of the pool (a third of ALLOC) is orphaned zvols** — 21 zvols on storage01 have no corresponding PV in the cluster, left behind by failed CSI deletions despite `reclaimPolicy: Delete`. Found while scoping the migration (post-incident). This was a major contributor to the pool filling. Runbook Phase 0 (cleanup still pending). The deletion-failure root cause — a corrupt 0-byte `/root/.nvmetcli/prefs.bin` crashing every nvmetcli invocation since May 19 — was found and fixed 2026-06-12; see runbook 0.4.
